...
-- as a single JSONB document (`plan`). Team 1's skill-analysis JSON is passed
-- in the request body at generation time and is NOT persisted here, so there
-- is no skill_profiles table and no roadmap_courses join table.
